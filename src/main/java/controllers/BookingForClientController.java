...
public class BookingForClientController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (error != null) {
            switch (error) {
                ...
                case "fail2":
                    flag = "fail2";
                    mes = "Phòng đã được đặt vào ngày này";
                    break;
                default:
                    flag = "";
                    mes = "";
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean isRoomBooked = bookingImpl.isRoomBooked(roomId, bookingStartDate, bookingEndDate);
        bookingImpl.releaseConnection();

        if (isRoomBooked) {
            // Nếu phòng đã được đặt, gửi thông báo và truyền các ngày đã đặt vào JSP
            ArrayList<Date> bookedDates = bookingImpl.getBookedDatesByRoomId(roomId);
            request.setAttribute("bookedDates", bookedDates);
            response.sendRedirect(request.getContextPath() + "/bookingforclient?room_id=" + roomIdParam + "&err=fail2");
            return;
        }
        ...
        if (bookingStartDate.compareTo(currentDate) < 0) {
            response.sendRedirect(request.getContextPath() + "/bookingforclient?room_id=" + roomIdParam + "&err=param");
            return;
        }
        ...
        for (Date date : bookedDates) {
            formattedDates.add(sdf.format(date));  // Thêm ngày đã định dạng vào danh sách
        }

        request.setAttribute("bookedDates", formattedDates);
